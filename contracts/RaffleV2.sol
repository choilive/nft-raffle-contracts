...
contract Raffle is Ownable, AccessControl, ReentrancyGuard, BaseRelayRecipient {
  ...
  /**
    ...
    */
  function cancelRaffle(uint256 raffleID) public onlyRole(DEFAULT_ADMIN_ROLE) {
    if (raffles[raffleID].endTime < block.timestamp) revert RaffleHasEnded(); // check this logic
  ...
  function claimTokenRewards(uint256 raffleID, address donor) public {
    ...
    //TODO
    // for (uint256 i = 0; i < donorsArray.length; i++) {
    //   uint donationPerAddresses = getTotalDonationPerAddressPerCycle(raffleID, donorsArray[i]);
    //  allDonationsPerAddresses.push(donationPerAddresses)
    // };
    // call rewards calculation contract
    uint256 amountToPay = ITokenRewardsCalculation(tokenRewardsModuleAddress)
      .calculateUserRewards(
        rewardTokenBalanceInContract,
        totalUserDonation,
        donorsArray,
    ...
    rewardTokenBalanceInContract -= amountToPay;
