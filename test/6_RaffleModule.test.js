...
describe("Raffle Module Tests", function () {
  ...
  describe("Create raffle function", function () {
    ...
    it("creates raffle with correct details", async () => {
      ...
      expect(await raffle.minimumDonationAmount).to.equal(
        ethers.utils.parseUnits("25", 6)
      );
      expect(await raffle.topDonor).to.equal(owner.address);
      expect(await raffle.topDonatedAmount).to.equal(
        ethers.utils.parseUnits("25", 6)
      );
      expect(await raffle.tokenAllocation).to.equal(1000);
      expect(await raffle.tokenBuffer).to.equal(1000);
    });
    ...
    it("contract receives NFTs on raffle creation", async () => {
      let contractNFTBalance = await NFTInstance.balanceOf(
  ...
  describe("cancelRaffle tests", function () {
    ...
    it("reverts if raffle has ended", async () => {
      ...
      await expect(
        RaffleInstance.connect(owner).cancelRaffle(1)
    ...
    it("updates donorsArray correctly", async () => {
      let donorsArray = await RaffleInstance.connect(owner).getDonorsPerCycle(
        1
      );
    ...
    it("refunds donors USDC correctly", async () => {
      ...
      expect(donor1BalAfterDonation).to.equal(
        donor1BalBefore.sub(ethers.utils.parseUnits("200", 6))
      );
      expect(donor2BalAfterDonation).to.equal(
        donor2BalBefore.sub(ethers.utils.parseUnits("300", 6))
      );
      ...
      let protocolFeePercent = await WrapperInstance.connect(
        owner
      ).getProtocolFee();
      let donation1ProtocolFee =
        (ethers.utils.parseUnits("200", 6) * protocolFeePercent) / 100;
      let donation2ProtocolFee =
        (ethers.utils.parseUnits("300", 6) * protocolFeePercent) / 100;
      let donation3ProtocolFee =
        (ethers.utils.parseUnits("200", 6) * protocolFeePercent) / 100;
      let donation4ProtocolFee =
        (ethers.utils.parseUnits("300", 6) * protocolFeePercent) / 100;
      ...
      let donor1Refund =
        ethers.utils.parseUnits("400", 6) - donor1TotalProtocolFee;
      let donor2Refund =
        ethers.utils.parseUnits("600", 6) - donor2TotalProtocolFee;

      expect(donor1BalAfterRefund).to.equal(
        donor1BalAfterDonation.add(donor1Refund)
      );
      expect(donor2BalAfterRefund).to.equal(
        donor2BalAfterDonation.add(donor2Refund)
      );
    });
    it("transfers NFTs back to author", async () => {
      expect(await NFTInstance.balanceOf(RaffleInstance.address, 2)).to.equal(
        0
      );
    ...
    it("sends rewardTokens to orgaisationWallet", async () => {
      ...
      expect(
        await RewardTokenInstance.balanceOf(RaffleInstance.address)
      ).to.equal(1000);
      expect(
        await RewardTokenInstance.balanceOf(organisationWalletAddress)
      ).to.equal(29000);
  ...
  describe("donate", function () {
    ...
    it("donation processes correctly", async () => {
      ...
      let treasuryAddress = await WrapperInstance.connect(
        owner
      ).getTreasuryAddress(organizationID);
      ...
      expect(await USDC.balanceOf(treasuryAddress)).to.equal(
        ethers.utils.parseUnits("200", 6).sub(protocolFee)
      );
    });
    ...
    it("handles donations correctly for multiple raffles", async () => {
      ...
      let donorsPerCycleArray = await RaffleInstance.connect(
        owner
      ).getDonorsPerCycle(1);
      ...
      let totalDonationsPerCycle = await RaffleInstance.connect(
        owner
      ).getTotalDonationsPerCycle(1);
      expect(totalDonationsPerCycle).to.equal(
        ethers.utils.parseUnits("620", 6)
      );

      let donor1Total = await RaffleInstance.connect(
        owner
      ).getTotalDonationPerAddressPerCycle(1, donor1Address);
      let donor2Total = await RaffleInstance.connect(
        owner
      ).getTotalDonationPerAddressPerCycle(1, donor2Address);
      ...
      donorsPerCycleArray = await RaffleInstance.connect(
        owner
      ).getDonorsPerCycle(2);
      ...
      totalDonationsPerCycle = await RaffleInstance.connect(
        owner
      ).getTotalDonationsPerCycle(2);
      expect(totalDonationsPerCycle).to.equal(
        ethers.utils.parseUnits("350", 6)
      );

      donor1Total = await RaffleInstance.connect(
        owner
      ).getTotalDonationPerAddressPerCycle(2, donor1Address);
      donor3Total = await RaffleInstance.connect(
        owner
      ).getTotalDonationPerAddressPerCycle(2, donor3Address);
  ...
  describe("TurnOnTokenRewards", function () {
    ...
    it("reverts on zero address", async () => {
      await expect(
        RaffleInstance.connect(owner).turnOnTokenRewards(
  ...
  describe("SendRewards function", function () {
    ...
    it("calculates winners correctly, NFT reflect in winners balances", async () => {
      ...
      expect(
        await NFTInstance.balanceOf(organisationWalletAddress, 1)
      ).to.equal(1);
  ...
  describe("withdraw function tests", function () {
    ...
    it("reverts if insufficient amount is available", async () => {
      await expect(
        RaffleInstance.connect(owner).withdraw(ownerAddress, 2050)
  ...
  describe("View functions", function () {
    ...
    it("getTokenBuffer", async () => {
      expect(await RaffleInstance.connect(owner).getTokenBuffer(1)).to.equal(
        1000
      );
    });
    ...
    it("getTokensInTheBufferEndOfCycle", async () => {
      expect(
        await RaffleInstance.connect(owner).getTokensInTheBufferEndOfCycle(1)
      ).to.equal(1000);
